package com.wang.choosephoto.adapter;
...
import com.wang.choosephoto.utils.MetricsUtils;
...
public abstract class BaseGroupAdapter<T> {
    ...
    private BaseViewItemHolder<T> mSelectorHolder;
        ...
        if (getCount() > 0) {
            if (mNum > 1) {
                for (int i = 0, size = getCount(); i < size; i += mNum) {
                    ...
                    int itemNum = mNum + i;
                    for (int j = i, num = Math.min(itemNum, size); j < num; j++) {
                        BaseViewItemHolder<T> holder = OnCreateViewHolder(mViewGroup);
    ...
    public abstract BaseViewItemHolder<T> OnCreateViewHolder(ViewGroup parent);

    public void OnBindViewHolder(final BaseViewItemHolder<T> holder, int position) {
    ...
    public BaseViewItemHolder<T> getSelectorHolder() {
        return mSelectorHolder;
    }

    public void setSelectorHolder(BaseViewItemHolder<T> selectorHolder) {
